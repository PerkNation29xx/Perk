...
def test_arcadia_guide_is_substantial_source_backed_and_reader_facing() -> None:
    ...
    assert "Arcadia has three current August outings" in html
...
def test_arcadia_routes_image_homepage_cards_and_sitemaps() -> None:
    ...
    for route in (
        ...
        assert "3 ranked outings" in response.text
    ...
    for route in ("/", "/white/"):
        response = client.get(route)
        assert response.status_code == 200
        assert response.text.count("Updated August 16 · Arcadia") == 1
        assert "Arcadia has three current August outings worth building an evening around." in response.text
...
def test_arcadia_guide_replaces_expired_roundup_copy_and_scopes_public_answer() -> None:
    ...
    assert answer
    assert "ranks three August outings" in answer
