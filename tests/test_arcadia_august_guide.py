...
def test_arcadia_guide_is_substantial_source_backed_and_reader_facing() -> None:
    ...
    assert "Arcadia has two remaining August outings" in html
...
def test_arcadia_routes_image_homepage_cards_and_sitemaps() -> None:
    ...
    for route in (
        ...
        assert "2 ranked outings" in response.text
    ...
    for route in ("/", "/white/"):
        response = client.get(route)
        assert response.status_code == 200
        assert response.text.count("Updated August 21 · Arcadia") == 1
        assert "Arcadia has two remaining August outings worth building an evening around." in response.text
        assert "Updated August 21" in response.text
...
def test_arcadia_guide_replaces_expired_roundup_copy_and_scopes_public_answer() -> None:
    ...
    assert 'dateModified": "2026-08-21"' in august_html
    ...
    assert answer
    assert "ranks two August outings" in answer
