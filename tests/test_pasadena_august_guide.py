...
def test_pasadena_routes_image_homepage_cards_and_sitemaps() -> None:
    ...
    for route in ("/", "/white/"):
        response = client.get(route)
        assert response.status_code == 200
        assert response.text.count("Updated August 28 · Pasadena") == 1
    ...
    assert "<loc>https://perknation.app/articles/pasadena-august-2026-guide</loc>" not in root_sitemap.text
