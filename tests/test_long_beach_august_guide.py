...
def test_long_beach_guide_is_substantial_source_backed_and_reader_facing() -> None:
    ...
    assert "Long Beach has five late-August plans" in html
    ...
    for expected in (
        ...
        "Naples Island Concert in the Park",
        "Conscience",
        "Best for:",
...
def test_long_beach_routes_image_homepage_cards_and_sitemaps() -> None:
    ...
    for route in ("/", "/white/"):
        response = client.get(route)
        assert response.status_code == 200
        assert response.text.count("Updated August 25 · Long Beach") == 1
        assert "Long Beach has five late-August plans worth building a day around." in response.text
...
def test_long_beach_guide_is_cross_linked_and_available_to_public_answers() -> None:
    ...
    assert answer
    assert "ranks five late-August plans" in answer
